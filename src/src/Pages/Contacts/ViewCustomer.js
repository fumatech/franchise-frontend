...
const ViewCustomer = () => {
  ...
  const filteredSales = safeSales.filter(
    (sale) =>
      moment(sale.saleDate).isSameOrAfter(startDate, "day") &&
      moment(sale.saleDate).isSameOrBefore(endDate, "day")
  );
    ...
    if (customer) {
      try {
        const saleResponse = await axios.get(
          `${process.env.REACT_APP_BASE_URL}/sale-combined-orders/getbycustomer/${customer.firstname}`
        );
        ...
        const paymentResponse = await axios.get(
          `${process.env.REACT_APP_BASE_URL}/payment-account/getbycustomer/${customer.firstname}`
        );
  ...
  const renderLedger = () => {
    ...
    if (sales && payments) {
      const filteredSales = sales.filter((sale) =>
        moment(sale.saleDate).isBetween(startDate, endDate, null, "[]")
      );
      ...
      <div className="card">
        <div className="card-body">
          ...
          <div className="mb-4">
            ...
            {renderRow("Name", `${prefix} ${firstname} ${lastname}`)}
            {renderRow("Email", email)}
            {renderRow("Customer ID", vendorId)}
            {renderRow("Firm Name", firmName)}
            {renderRow("GST Number", taxOrGstNumber)}
            {renderRow("Shop Act Number", shopActNumber)}
            {renderRow("CIN Number", cinNumber)}
            {renderRow("PAN Number", panNumber)}
            {renderRow("Status", isActive ? "Active" : "Inactive")}
          </div>

          <div className="mb-4">
            <h5 className="mb-3">Personal Information</h5>
            {renderRow("Username", username)}
            {renderRow("Allow Login", allowLogin ? "Yes" : "No")}
            ...
            {renderRow("Blood Group", bloodGroup)}
            {renderRow("Mobile Number", mobileNumber)}
            {renderRow("Alternate Contact", alternateContactNumber)}
            {renderRow("Family Contact", familyContactNumber)}
          </div>

          <div className="mb-4">
            <h5 className="mb-3">Social Media</h5>
            {renderRow("Facebook", facebookLink)}
            {renderRow("Twitter", twitterLink)}
            {renderRow("Social Media 1", socialMedia1)}
            {renderRow("Social Media 2", socialMedia2)}
          </div>

          <div className="mb-4">
            <h5 className="mb-3">Custom Fields</h5>
            {renderRow("Custom Field 1", customField1)}
            {renderRow("Custom Field 2", customField2)}
            {renderRow("Custom Field 3", customField3)}
            {renderRow("Custom Field 4", customField4)}
          </div>

          <div className="mb-4">
            <h5 className="mb-3">Identification</h5>
            {renderRow("Guardian Name", guardianName)}
            {renderRow("ID Proof", `${idProofName} (${idProofNumber})`)}
          </div>

          <div className="mb-4">
            <h5 className="mb-3">Address</h5>
            {renderRow("Permanent Address", permanentAddress)}
            {renderRow("Current Address", currentAddress)}
            {renderRow("Country", country)}
            {renderRow("State", state)}
            {renderRow("City", city)}
            {renderRow("Zip Code", zipCode)}
          </div>

          <div className="mb-4">
            <h5 className="mb-3">Bank Details</h5>
            {renderRow("Account Holder Name", accountHolderName)}
            {renderRow("Account Number", accountNumber)}
            {renderRow("Bank Name", bankName)}
            {renderRow("IFSC Code", ifsc)}
            {renderRow("Branch", branch)}
            {renderRow("Tax Payer ID", taxPayerId)}
          </div>
        </div>
      </div>
    );
