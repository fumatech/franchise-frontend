...
const ViewUser = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (userData.bankFields && userData.bankFields.length >= 6) {
        const getValue = (field) => (field ? field.split(":")[1] || "" : "");
        setAccountHolderName(getValue(userData.bankFields[0]));
        setAccountNumber(getValue(userData.bankFields[1]));
        setBankName(getValue(userData.bankFields[2]));
        setBankCode(getValue(userData.bankFields[3]));
        setBranch(getValue(userData.bankFields[4]));
        setTaxPayerId(getValue(userData.bankFields[5]));
      }
  ...
  const fetchPayComponents = () => {
    api
      .get("/pay-component/all")
      .then((response) => setPayComponents(response.data))
  ...
  const checkEmailExists = async (email) => {
    try {
      ...
      return data.id !== parseInt(id); // true if email exists for another user
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const userData = {
      ...
      commisionPercent: salesCommissionPercentage,
