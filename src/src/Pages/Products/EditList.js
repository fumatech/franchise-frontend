...
function EditList() {
  ...
  {
    /*Amount count as per tax function start */
  }

  const calculateIncTaxFromExc = (excTaxPrice) => {
    const taxRate = applicableTax
      ? parseFloat(applicableTax.match(/\d+/)) / 100
      : 0;
  ...
  const handleApplicableTaxChange = (selectedTax) => {
    setApplicableTax(selectedTax); // Update the applicable tax
    recalculatePrices(selectedTax); // Trigger price recalculations
  };
  ...
  // Function to recalculate prices based on the selected tax and other fields
  const recalculatePrices = (selectedTax) => {
    const taxRate = getTaxRate(selectedTax); // Get the tax rate from selectedTax
    const excTaxPrice = parseFloat(defaultPurchasePriceExcTax) || 0;

    // Calculate Inc. Tax based on Exc. Tax and applicable tax rate
    const newIncTaxPrice = excTaxPrice * (1 + taxRate);
    setDefaultPurchasePriceIncTax(newIncTaxPrice.toFixed(2)); // Update Inc. Tax price

    // Now update the default selling price based on the current tax type
    const profitMarginValue = parseFloat(profitMargin) || 0;
    let sellingPrice = 0;

    if (sellingPriceTaxType === "Exclusive") {
      sellingPrice = excTaxPrice * (1 + profitMarginValue / 100); // For Exclusive Tax
    } else if (sellingPriceTaxType === "Inclusive") {
      sellingPrice = newIncTaxPrice * (1 + profitMarginValue / 100); // For Inclusive Tax
    }

    setDefaultSellingPrice(sellingPrice.toFixed(2)); // Update Selling Price
  };
  ...
  {
    /*Amount count as per tax function end */
  }

  {
    /*Amount count as per tax variable function start */
  }
  ...
  {
    /*Amount count as per tax variable function end */
  }

  {
    /*Variable product function start */
  }
  ...
  // Function to handle changes in specific row fields
  const handleRowChange = (e, variationIndex, rowIndex, field) => {
    const value = e.target.value;
    setVariations((prevVariations) => {
      const updatedVariations = [...prevVariations];
      updatedVariations[variationIndex].rows[rowIndex][field] = value;
      return updatedVariations;
    });
  };
  ...
  {
    /*Variable product function end */
  }

  {
    /*Combo product function start */
  }
    ...
    // Handle VARIABLE product type
      ...
      variations.forEach((variation) => {
        variation.rows.forEach((row) => {
          ...
          // Collect variation images specifically for the current row
          if (row.variationProductImages) {
            row.variationProductImages.forEach((image) => {
              // Append each image without using an index in the key
              formData.append("variationImages", image); // Use the same key for all images
            });
          }
        });
      });
                                ...
                                <div className="">
                                  ...
                                  <div className="d-flex align-items-center">
                                    ...
                                    >
                                      ...
                                      <option value="CGST@10%">CGST@10%</option>
                                      <option value="SGST@8%">SGST@8%</option>
