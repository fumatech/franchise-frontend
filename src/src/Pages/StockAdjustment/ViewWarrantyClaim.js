    ...
    const fetchStockAdjustmentData = async () => {
      try {
        ...
        const response = await axios.get(
          `https://fusionmastertech.com:8443/franchise-warranty-claim/get/${id}`
        );
        const data = response.data;

        setAdjustmentDate(data.date);
        setBusinessLocation(data.vendor);
        ...
        const selectedProducts = data.franchiseWarrantyClaimItems.map(
          (item) => ({
            id: item.id,
        ...
        data.franchiseWarrantyClaimItems.forEach((item) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const saveResponse = await fetch(
        `${process.env.REACT_APP_BASE_URL}/warranty-claim/save`,
        {
          method: "POST",
          headers: {
      ...
      const savedWarrantyClaim = await saveResponse.json(); // Get the saved claim details

      // Use the id from the URL params directly for the update
      const updateResponse = await fetch(
        `${process.env.REACT_APP_BASE_URL}/warranty-claim/update/${id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      const updatedWarrantyClaim = await updateResponse.json(); // Get the updated claim details

      // Handle successful response
      alert("Warranty Claim updated successfully.");
      // Optionally, navigate to another page or reset form
      // navigate("/stock-adjustments"); // Uncomment to redirect if needed
    } catch (error) {
      setError(error.message); // Set error message in the UI
      console.error(error);
    } finally {
                  ...
                  <div className="card-body">
                    <div className="row">
                      <div className="form-group col-md-3">
                        <label htmlFor="businessLocation">Vendor:*</label>
                        <select
                          id="businessLocation"
                      ...
                      <div className="col-md-12">
                        ...
                        {selectedProducts.length > 0 && (
                          <div className="table-responsive">
                            ...
                            <div>Accepted Total Amount: ₹{totalAmount}</div>
                            <div> Accepted Total Units : {totalUnits}</div>
                          </div>
                        )}
                      </div>
                    </div>
