...
const ExpenseCategories = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/expenses/getall`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.id) {
        ...
        await axios.put(
          `${process.env.REACT_APP_BASE_URL}/expenses/update/${formData.id}`,
          categoryData
        );
        ...
        // Create new category
        await axios.post(
          process.env.REACT_APP_BASE_URL + "/expenses/save",
          categoryData
        );
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this category?")) {
      try {
        await axios.delete(
          `${process.env.REACT_APP_BASE_URL}/expenses/delete/${id}`
        );
