...
function AddWarrantyClaim() {
  ...
  const handleSubmit = async (event) => {
    ...
    const stockAdjustmentData = {
      franchise: franchiseId, // Add the franchise field
      vendor,
      referenceNumber,
      date: adjustmentDate.toISOString(),
      status: 0, // Default status for new claim
      totalAmount,
      totalUnits,
      reason,
      franchiseWarrantyClaimItems: selectedProducts.map((product) => ({
      ...
      stockTransaction: selectedProducts.map((product) => ({
        ...
        date: adjustmentDate.toISOString(),
        note: "Stock added to warehouse as warranty_claimed",
      })),
    };

    try {
      const response = await fetch(
        `https://fusionmastertech.com:8443/franchise-warranty-claim/save`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errMsg || "Failed to save franchise warranty claim");
      }

      alert("Franchise Warranty Claimed Successfully.");
      navigate("/ListWarrantyClaim"); // Redirect to claims list
    } catch (error) {
      setError("Error saving franchise warranty claim: " + error.message);
      console.error(error);
    } finally {
      setLoading(false);
    }
