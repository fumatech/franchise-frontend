...
function AddSaleReturn() {
    ...
    setSelectedProducts((prev) =>
      prev.map((product) =>
        product.id === productId
          ? {
              ...product,
              taxRate: selectedOption ? selectedOption.rate : 0, // Store the rate for calculations
              taxRateId: selectedOption ? selectedOption.value : null, // Store the ID for backend
            }
          : product
      )
    );
  ...
  useEffect(() => {
    // Initialize subtotal and total units
    let subtotal = 0;
    let totalUnits = 0;
    ...
    selectedProducts.forEach((product) => {
      ...
      // Skip calculations if quantity is 0 (do not add to subtotal)
      if (quantity === 0 || unitCostBeforeDiscount === 0) {
        return; // Skip this product's calculations if quantity or unit cost is 0
      }

      // Line Total (without tax)
      const lineTotal = unitCostBeforeDiscount * quantity;

      // Tax Calculation (Based on Line Total)
      const taxRate = product.taxRate || 0;
      const taxAmount = lineTotal * (taxRate / 100) || 0;

      // Line Total with Tax
      const lineTotalWithTax = lineTotal + taxAmount;

      // Add to subtotal and total units
      subtotal += lineTotalWithTax;
      totalUnits += quantity;
    });

    // Check if subtotal is still NaN or invalid after calculations
    if (isNaN(subtotal)) {
      console.error("Error: Subtotal calculation resulted in NaN.");
      return; // Exit early to avoid setting invalid values
    }

    setSubTotalAmount(subtotal.toFixed(2));

    // Calculate the total discount (fixed or percentage)
    let totalDiscount = 0;
    ...
    const subTotals = subtotal - totalDiscount;

    // Tax Calculation on subtotal after discount
    const selectedTax = taxOptions.find((opt) => opt.value === purchaseTax);
    const taxRate = selectedTax?.rate || 0;
    const taxAmountOnSubtotal = (subTotals * taxRate) / 100;
    setTaxOnsubtotal(taxAmountOnSubtotal);

    // Final Total Amount including Shipping and Additional Expenses
    ...
    const finalAmount =
      subTotals + shipping + taxAmountOnSubtotal + additionalExpensesTotal;
  ...
  useEffect(() => {
    if (selectedOrderId) {
      axios
        .get(
          `${process.env.REACT_APP_BASE_URL}/sale/getSaleDataById/${selectedOrderId.value}`
        )
        .then((response) => {
          ...
          setReferenceNumber(data.invoiceNo);
          setOrderedBy(data.addedBy);
          // setOrderDate(new Date(data.orderDate));
          setLocation(data.location);
          setAdditionalNotes(data.additionalNotes);
          ...
          setFinalPurchaseAmount(data.netTotalAmount);
          ...
          const selectedProducts = data.saleItems.map((item) => {
          ...
          setProductsData(data.saleItems);
        })
        .catch((error) => {
          console.error("Error fetching purchase order data:", error);
        });
    }
  }, [selectedOrderId, taxOptions]);

  useEffect(() => {
    // Fetch order IDs from backend
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/sale/getAllOrderIds`)
      .then((response) => {
  ...
  useEffect(() => {
    ...
    if (discountType === "Fixed") {
      discountAmountValue = Math.min(discountInput, subtotal);
    } else if (discountType === "Percentage") {
      discountAmountValue = (subtotal * discountInput) / 100;
    }

    const amountAfterDiscount = subtotal - discountAmountValue;

    // Calculate tax on subtotal after discount
    const selectedTax = taxOptions.find((opt) => opt.value === purchaseTax);
    const taxRateOnSubtotal = selectedTax?.rate || 0;
    const taxOnSubtotalValue = (amountAfterDiscount * taxRateOnSubtotal) / 100;

    // Calculate shipping and additional expenses
    const shipping = parseFloat(shippingCharges) || 0;
    const additionalExpensesTotal = additionalExpenses.reduce(
      (sum, expense) => sum + (parseFloat(expense.amount) || 0),
      0
    ...
    // Update states
    setSubTotalAmount(subtotal.toFixed(2));
    setTaxOnsubtotal(taxOnSubtotalValue.toFixed(2));
    setFinalPurchaseAmount(finalAmount.toFixed(2));
  }, [
    selectedProducts,
    discountType,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post(
        `${process.env.REACT_APP_BASE_URL}/saleReturn/save`,
        payload,
        {
          headers: {
            ...
            <div className="container-fluid">
              <div className="row mb-2">
                <div className="col-sm-6">
                  <h1 className="all-heading">Add Sale return</h1>
                </div>
              </div>
            </div>
          </section>
                          ...
                          <div>
                            ...
                            <div className="d-flex align-items-center">
                              <Select
                                ...
                                onChange={(selected) => {
                                  setSelectedProducts([]); // Reset products when order changes
                                  setSelectedOrderId(selected); // Update selected order
                                }}
                      ...
                      <div className="col-md-4">
                        <div className="form-group d-flex flex-row flex-md-column">
                          ...
                          <DatePicker
                            ...
                            onChange={(date) => setPurchaseDate(date)} // Ensure `date` is a Date object
                            className="form-control w-100 ms-1 ms-md-0 py-3 rounded-1"
                            dateFormat="dd/MM/yyyy"
                            required
                            minDate={new Date()} // Prevent past dates
