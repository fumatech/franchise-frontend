...
const AddUser = () => {
  ...
  const checkEmailExists = async (email) => {
    try {
      ...
      return false; // Gracefully handle error
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const userData = {
      ...
      password,
      allowLogin,
      enableServiceStaffPin: enableServiceStaffPin,
      staffPin,
      roles: selectedRoleId ? [{ id: selectedRoleId }] : [],
      language,
      dateOfBirth,
      gender,
      maritalStatus,
    ...
    try {
      // Tenant header is automatically added by api instance
      ...
      // If it's a session/tenant issue, redirect to login
      if (error.response?.status === 401 || error.message.includes("tenant")) {
