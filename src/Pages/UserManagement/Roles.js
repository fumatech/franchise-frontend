  ...
  useEffect(() => {
    api
      ...
      .catch((error) => {
        ...
        // Optional: redirect to login if no tenant or unauthorized
      ...
      api
        ...
        .catch((error) => {
          ...
          if (
            ...
            error.message.includes("No tenant")
          ) {
            localStorage.removeItem("tenantDbName");
            sessionStorage.removeItem("tenantDbName");
            alert("No tenant database selected. Please login again.");
            navigate("/login"); // Only if you're using `useNavigate` from react-router
          } else {
                  ...
                  >
                    ...
                    <tbody>
                      {displayedRoles.map((role) => {
                        const isSuperAdmin = role.role === "Admin"; // 👈 Check if role is Admin
                            ...
                            {columnsVisibility.actions && (
                              <td className="">
                                <div className="btn-group btn-group-sm btn-icon-only">
                                  {!isSuperAdmin && ( // 👈 Only show buttons if NOT Admin
                                    <>
                                      <button
                                        type="button"
                                        className="btn-edit"
