...
const InputTaxPurchase = () => {
  ...
  const exportCSV = () => {
    ...
    const csvData = inputTaxPurchase.map((order) => {
    ...
    saveAs(new Blob([csv], { type: "text/csv" }), "InputTaxPurchase.csv");
  };
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(ws, wb, "InputTaxPurchase");
  ...
  const exportPDF = () => {
    ...
    const body = inputTaxPurchase.map((order) => {
      const orderTaxes = getOrderTaxAmounts(order);
      return [
        order.orderDate,
        order.referenceNumber,
                      ...
                      <tr>
                        <td
                          colSpan={
                            ...
                            (columnsVisibility.supplier ? 1 : 0)
                          }
                        >
